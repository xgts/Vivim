...
class SalObjDataset(data.Dataset):
    def __init__(self, root, trainsize, clip_len=5):
        ...
        image_root = os.path.join(root, "train", "videos")
        vid_list = os.listdir(image_root)
        vid_list = sorted(vid_list, key=lambda x: int(x[3:]))
        ...
        for vid in vid_list:
            ...
            for idx in range(len(frms)):
                ...
                self.gts.append([x.replace("videos", "gts").replace("jpg", "png") for x in clip])
    ...
    def resize(self, img, gt):
        ...
        if h < self.trainsize or w < self.trainsize:
            h = min(h, self.trainsize)
            w = min(w, self.trainsize)
...
class SalObjTestDataset(data.Dataset):
    def __init__(self, root, trainsize, clip_len=5):
        self.trainsize = trainsize
        image_root = os.path.join('./data', "val", "videos")
        vid_list = os.listdir(image_root)
        vid_list = sorted(vid_list, key=lambda x: int(x[3:]))
        self.clip_len = clip_len
        self.images = []
